...
**Current as of:** 2026-08-22 (M3g final recovery review). Read `INSTRUCTIONS.md` first; `BETA_PLAN.md` is the living plan.
...
- **History (all ff-merged into the trunk):** `1833331` Qt 6.11 fix → `db8563d` AI-Skills → BETA_M1 (M1a–M1e) → BETA_M2 (M2a–M2e, `dc125d8`) → M3a–M3f (`d4ea8c2`+`5abc800`) → M4a `cffb7cd`+`f3376c7` → M4b `eb83966`+`e6152bd` → M4c `e5d69e0`+`520b2d7` → M4d `0d8a883` → M3g recovery (this branch). Full per-slice detail lives in BETA_PLAN.md's change log.
...
| 2026-08-22 | Codex recovery implementer + adversarial review | **BETA_M3g final security review APPROVE**: file videos snapshot through a validated descriptor; destination slots use stable directory-fd `openat`/`unlinkat`; the source cap probes an extra byte without writing; RAII and supervisor cleanup cover staged media; smoke teardown and asynchronous pixel waits are bounded. Targeted scene 172 and daemon 128 tests passed; scene and daemon clippy passed. The orchestrator's traced `scripts/smoke-scene.sh` passed all M3g daemon/package/corrupt/media/cleanup cases and standalone llvmpipe lanes, with the Plasma PID unchanged. Prior smoke-video and smoke-supervisor gates were green. No new full-workspace/check claim; no UI or live-Plasma changes. | M3g gate evidence complete; M3h next |
