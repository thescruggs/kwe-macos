...
impl GlyphAtlas {
    ...
    /// Drop all content (used when the font or size changes). Not counted
    /// against the rebuild rate limit: a user-initiated change must never
    /// starve itself.
...
pub fn build_vertex_bytes(
    ...
    for p in &layout.placements {
        ...
        let u0 = rect.x as f32 * inv;
        let v0 = rect.y as f32 * inv;
        let u1 = (rect.x + rect.w) as f32 * inv;
        let v1 = (rect.y + rect.h) as f32 * inv;
...
impl TextRenderer {
    ...
    pub fn sync_and_upload(
        ...
        for (i, layer) in layers.iter().enumerate() {
            ...
            // Upload (only when something changed this pass).
            let mut ok = true;
...
mod tests {
    ...
    fn vertex_bytes_match_unit_quad_winding() {
        ...
        let u0 = 32.0 / ATLAS_SIZE as f32;
        let v0 = 64.0 / ATLAS_SIZE as f32;
        let u1 = 54.0 / ATLAS_SIZE as f32;
        let v1 = 76.0 / ATLAS_SIZE as f32;
