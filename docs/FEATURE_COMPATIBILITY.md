...
| `playlist.ordered-shuffle` | Ordered/shuffled playlists | P0 | M5g–M5h provide bounded ordered/seeded-shuffle selection, no immediate repeat, deterministic unavailable/quarantine skipping, and bounded runtime history; persistence and renderer assignment remain open. |
| `playlist.timer` | Duration and wallpaper transitions | P0 | M5g–M5h persist bounded settings and implement monotonic pause-aware decisions; daemon restart recovery and renderer transitions remain open. |
