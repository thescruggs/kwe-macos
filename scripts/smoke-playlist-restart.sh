...
{"title":"Synthetic One","type":"video"}
EOF
}
...
{"title":"Synthetic Three","type":"video"}
EOF
call_daemon rescan >/dev/null
...
before_remaining="$(jq -r '.result.decision.remaining_ms' <<<"$status")"
...
for _attempt in {1..250}; do
    ...
    [[ "$(jq -r '.result.phase' <<<"$renderer_status")" == "quarantined" ]] && break
    sleep 0.02
done
[[ "$(jq -r '.result.phase' <<<"$renderer_status")" == "quarantined" ]] \
    || fail "renderer 1 must reach quarantine"
